...
\begin{block}{Endividamento Populacional}
  
\scriptsize
O sistema financeiro expandiu as hipotecas subprime, concedidas a famílias de baixa renda com condições iniciais atrativas. Com a alta dos juros e a queda dos preços dos imóveis, muitos mutuários tornaram-se inadimplentes, comprometendo as garantias dos empréstimos.
\end{block}
\column{0.32\textwidth}

\begin{alertblock}{Colapso Bancario}
\scriptsize
Quando a bolha imobiliária finalmente estourou, a quedra do  banco Lehman Brother serviu como gatilho para um colapso global. Como inúmeros títulos de dívida estavam vinculados a essa instituição, o sistema financeiro desmoronou rapidamente, provocando uma derrocada nas bolsas e instalando o caos generalizado em Wall Street.
\end{alertblock}
\column{0.32\textwidth}

\begin{block}{Dificuldade na Recuperação}
\scriptsize
Com a confiança dos mercados severamente comprometida, a recuperação americana tornou-se incerta e desigual. O temor de uma recessão prolongada retraiu os investimentos, afetou drasticamente o poder de compra das famílias e forçou o país a enfrentar um longo período de ajuste financeiro.
\end{block}
\end{columns}
\end{frame}
\begin{frame}{Impactos Internos}
\framesubtitle{Situação Social}

\scriptsize

\begin{block}{Crise Imobiliária}

\begin{columns}[c]

\column{0.24\textwidth}
\centering
\includegraphics[width=\linewidth]{figuras/trailer.png}

\column{0.72\textwidth}
A cultura de viver em trailers já existia nos Estados Unidos, mas ganhou outra dimensão após a crise de 2008. Com a onda de despejos provocada pela execução de hipotecas, milhares de famílias perderam suas casas e passaram a utilizar habitações móveis como alternativa de moradia, dando origem ao fenômeno dos chamados \textit{nômades modernos}.

\end{columns}

\end{block}

\begin{block}{Aumento da População em Vulnerabilidade}

\begin{columns}[c]

\column{0.24\textwidth}
\centering
\includegraphics[width=\linewidth]{figuras/nyt.jpeg}

\column{0.72\textwidth}
A crise também elevou significativamente o número de pessoas em situação de vulnerabilidade. A perda de empregos e a execução das hipotecas levaram milhares de famílias à condição de sem-teto, agravando problemas sociais e ampliando a desigualdade.
...
\end{frame}
\begin{frame}{Impactos Externos}
\begin{columns}

\column{0.55\textwidth}
\includegraphics[width=\linewidth]{figuras/var_pib_mundo.png}

\column{0.42\textwidth}
\begin{block}
\scriptsize
Bancos, fundos de investimento e entidades governamentais na Europa e na Ásia haviam adquirido títulos ligados as entidades financeiras americanas. Quando a bolha estoura o valor desses ativos cai drasticamente, paralisando a oferta de credito mundial e travando investimentos. 
\end{block}
\end{columns}
\end{frame}
\begin{frame}{Impactos Externos}
  \framesubtitle{Regiões Mais afetadas}
  \begin{block}{Europa e Ásia}
\scriptsize
A crise provocou forte retração do comércio internacional. Na Europa Ocidental, as exportações caíram cerca de 20\%, afetando economias dependentes do mercado americano, como a Alemanha. Na Ásia, o impacto foi ainda maior: no Japão, as exportações chegaram a recuar aproximadamente 50\%, refletindo o colapso da demanda global.
\end{block}
\begin{block}{Leste Europeu}
\scriptsize
A interrupção do fluxo internacional de capitais atingiu fortemente países do Leste Europeu e da região do Báltico, que dependiam de financiamento externo. Sem acesso ao crédito, diversas economias recorreram a empréstimos do FMI, aceitando programas de austeridade que incluíram cortes de salários, pensões e gastos públicos.
\end{block}
\begin{block}{Crise da Dívida Soberana}
\scriptsize
Para evitar o colapso do sistema financeiro, diversos governos assumiram prejuízos privados e ampliaram seus gastos, elevando rapidamente a dívida pública. Como consequência, países como Grécia, Irlanda, Portugal e Espanha enfrentaram uma grave crise de dívida soberana, sendo obrigados a solicitar resgates financeiros e implementar severas políticas de austeridade.
\end{block}
\end{frame}
